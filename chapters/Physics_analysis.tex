...
\begin{itemize}
  ...
  \item \textbf{D$^*$ signal model}: For D$^*$ signal, the model was changed to a double Gaussian
  \item \textbf{D$^*$ background model}: the background was modified to another threshold function \cite{CMS:2021lab} with the form
  \begin{equation}
  ...
  \end{equation}
  where $m_\pi$ is taken as the pion mass and $p_0$, $p_1$ and $p_2$ are free parameters.
  \item \textbf{$\Upsilon$ signal model}: To evaluate this systematic, the shape of the CB is commonly changed to a Gaussian, but it can lead to bad fits, this models does not properly describe the tails in the dimuon invariant mass distribution. Therefore, rather than changing the signal model, 
  \item \textbf{$\Upsilon$ background model}
