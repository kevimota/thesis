...
\end{equation*}
adding to a total BR of 2.67 $\pm$ 0.03 \%. The mass difference of D$^{*+}$ and D$^0$ is small, so the momentum of the pion from the D$^{*+}$ is also small. For this reason, this pion is often referred to as ``slow'' pion ($\pi_s$).
...
\begin{feynman}
    ...
    \vertex[below left=of a] (i2) {\(\bar b\)};
    \vertex[right=of a] (b);
    \vertex[above right=of b] (i3) {\(\mu^+\)};
    \vertex[below right=of b] (i4) {\(\mu^-\)};

    \diagram* {
        (i1) -- [fermion] (a) -- [fermion] (i2),
        (a) -- [boson,  edge label=\(\gamma\)] (b),
        (i3) -- [fermion] (b) -- [fermion] (i4)
    };

    \draw [decoration={brace}, decorate] (i2.south west) -- (i1.north west) node [pos=0.5, left] {\(\Upsilon\)};
\end{feynman}
\end{tikzpicture}
}
\legend{Example of Feynman diagram for $b\bar b$ ($\Upsilon$) annihilation to dimuon.}
\end{figure}
